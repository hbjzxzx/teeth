...
from utils import *
from AtNets import ATMaskLoss, ClsWeightLoss,FocalLoss 
...
def main():
    ...
    optimizer = optim.SGD(model.parameters(), lr=1e-4, momentum=0.5, weight_decay=1e-3)
    ...
    for e in range(num_epochs):
        ...
        for i, data in enumerate(TrainDataloader):
            ...
            if i % 5  == 4:
                ...
                writer.add_pr_curve('Training Crack PR',torch.cat(running_cls_label), cls_prob, global_step=step)
                running_cls_label.clear()
                running_cls_prob.clear()

    torch.save(model.state_dict(), str(saveRoot/Path('model_final.pth')))

def test(model, testdata, device,writer, step, cri):
    pred_At = []
    running_cls_label = []
    running_cls_prob = []
    loss_record = 0.0
    ...
    with torch.no_grad():
        for data in testdata:
            ...
            clsout, outmask = net(inputs)
            loss = cri(clsout, labels)
    ...
    cls_prob = torch.cat(running_cls_prob, dim=0)
    cls_prob = F.softmax(cls_prob, dim=1)[:,1]
    writer.add_pr_curve('Testing Crack PR',torch.cat(running_cls_label), cls_prob, global_step=step)
    writer.add_scalar('Testing loss', loss_record/runS, global_step=step)
    running_cls_label.clear()
    running_cls_prob.clear()
